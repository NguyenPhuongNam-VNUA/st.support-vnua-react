import {
  ...
  function DuplicateQuestionsTable({ duplicateQuestions = [], onConfirm, onChangeAction }: any) {
    return (
      <Box>
        ...
        <Paper variant="outlined">
          <Table size="small">
            <TableHead sx={{ backgroundColor: '#f4f6f8' }}>
              <TableRow>
                <TableCell sx={{ color: '#333', padding: '1px' }} width="40">#</TableCell>
            ...
            <TableBody>
              {duplicateQuestions.map((q: any, index: number) => (
                <TableRow key={index} hover>
                  <TableCell sx={{ padding: '1px' }}>{index + 1}</TableCell>
                  <TableCell>
