...
class GraphRunner:
    ...
    async def astream_events(
        ...
        initial_state["streamed_deltas_count"] = 0

        async def _run_pipeline() -> None:
            nonlocal first_status_recorded
            try:
                ...
                await event_queue.put(SSEEvent(event="request.accepted", data=accepted).to_dict())
                    ...
                    async for item in self._astream_updates(initial_state):
                        ...
                        if label_info:
                            ...
                            if traces:
                                ...
                                raw_details = (
                                    latest.details
                                    if hasattr(latest, "details")
                                    else latest.get("details", {})
                                )
                            ...
                            await event_queue.put(
                                SSEEvent(event="pipeline.status", data=status_event).to_dict()
                            )
                else:
                    raise RuntimeError("Compiled LangGraph does not support asynchronous streaming")

                # If generation didn't stream deltas (e.g. cache hit or fallback template), emit answer now
                answer_text = current_state.get("answer", "")
                if current_state.get("streamed_deltas_count", 0) == 0 and answer_text:
                    await event_queue.put(
                        SSEEvent(
                            event="answer.delta",
                            data=AnswerDeltaPayload(request_id=req_id, delta=answer_text, index=0),
                        ).to_dict()
                    )

                # 3. Final Event: answer.completed
                ...
                await event_queue.put(SSEEvent(event="answer.error", data=err_payload).to_dict())
            finally:
