...
async def init_db_pool(settings: Optional[Settings] = None) -> asyncpg.Pool:
    ...
    try:
        ...
        raise DatabaseUnavailableError(
            message=f"Không thể khởi tạo pool kết nối PostgreSQL: {exc}"
        ) from exc
...
async def get_db_connection() -> AsyncGenerator[asyncpg.Connection, None]:
    ...
    try:
        ...
        raise DatabaseUnavailableError(message=f"Lỗi cơ sở dữ liệu: {pg_err}") from pg_err
    except Exception as exc:
        logger.error("Unexpected error acquiring/using DB connection: %s", exc)
        raise DatabaseUnavailableError(message=f"Lỗi kết nối cơ sở dữ liệu: {exc}") from exc
    finally:
